...
import { format, getMonth, isSameMonth, startOfWeek, endOfWeek, eachWeekOfInterval, startOfYear, endOfYear, addDays } from 'date-fns'
...
const WeekHeader: React.FC<WeekHeaderProps> = ({ weeks }) => {
    ...
    return (
        <thead>
            <tr>
                <th className="border-b border-r border-sky-700 p-2 bg-cyan-500 text-white left-0 z-20 font-semibold">
                    Products
                </th>
                {months.map((month) => (
                    <th
                        ...
                        className="border-b border-r border-sky-700 p-2 bg-cyan-500 text-white text-center font-normal"
                    >
                        {month.month}
                    </th>
                ))}
                ...
                {weeks.map((week) => {
                    ...
                    return (
                        <th
                            ...
                            title={`${format(week.startDate, 'MMM d')} - ${format(endDate, 'MMM d, yyyy')}`}
                        >
                            <div className="text-xs flex flex-col items-center">
                                <span className="font-medium">{format(week.startDate, 'dd')}</span>
                                <span className="text-[9px] text-gray-500">
                                    {isSameMonth(week.startDate, endDate)
                                        ? format(endDate, 'dd')
