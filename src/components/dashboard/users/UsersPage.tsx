...
interface User {
  ...
  role: 'Admin' | 'CXO' | 'Brand Manager' | 'Sales Manager';
  avatar?: string;
  lastLogin: string;
  permissions: string[];
  department?: string;
...
export default function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({
    ...
    role: 'Admin' as 'Admin' | 'CXO' | 'Brand Manager' | 'Sales Manager',
    department: '',
    location: ''
  });
  ...
  const handleAddUser = () => {
    ...
    const newUserData: User = {
      ...
      role: newUser.role,
      lastLogin: 'Never',
      permissions: newUser.role === 'Admin' ? ['full_access', 'user_management', 'billing', 'settings'] :
                  newUser.role === 'CXO' ? ['full_access', 'user_management', 'billing', 'settings'] : 
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'Admin':
        return <Badge className="bg-purple-100 text-purple-800 hover:bg-purple-200 hover:text-purple-900 text-xs whitespace-nowrap"><Shield className="w-3 h-3 mr-1" />Admin</Badge>;
      case 'CXO':
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 mt-2 text-lg">Manage your B2B companies, users, and subscriptions</p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="group relative overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl">
            ...
            <CardContent className="relative p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-blue-700 mb-1">Total Companies</p>
          ...
          <Card className="group relative overflow-hidden bg-gradient-to-br from-emerald-50 to-green-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl">
            ...
            <CardContent className="relative p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-emerald-700 mb-1">Active Companies</p>
          ...
          <Card className="group relative overflow-hidden bg-gradient-to-br from-red-50 to-pink-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl">
            ...
            <CardContent className="relative p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-red-700 mb-1">Admins</p>
                  <p className="text-3xl font-bold text-red-900">{users.filter(u => u.role === 'Admin').length}</p>
                  <p className="text-xs text-red-600 mt-1">Administrator accounts</p>
                </div>
          ...
          <CardHeader className="bg-gradient-to-r from-slate-50 to-gray-50 border-b border-gray-100/50 pb-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-xs">
                {filteredUsers.length} of {users.length} companies
              </Badge>
            </div>
          </CardHeader>
                ...
                <div className="bg-gradient-to-r from-slate-50 via-blue-50 to-indigo-50 border-b border-gray-200/80 px-8 py-6">
                  <div className="grid grid-cols-12 gap-8 items-center">
                    <div className="col-span-3">
                      <div className="flex items-center gap-3 cursor-pointer hover:text-blue-600 transition-colors group" onClick={() => handleSort('companyName')}>
                        ...
                        <div>
                          <span className="font-semibold text-gray-800 text-sm">Company Details</span>
                          {sortField === 'companyName' && (
                            <div className="flex items-center gap-1 mt-1">
                              {sortDirection === 'asc' ? <ChevronUp className="w-3 h-3 text-blue-600" /> : <ChevronDown className="w-3 h-3 text-blue-600" />}
                              <span className="text-xs text-blue-600">Sorted by Company</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="col-span-3 flex items-center gap-4 min-w-0 relative z-10">
                        <div className="relative">
                          <Avatar className="w-12 h-12 ring-4 ring-white group-hover:ring-blue-200 transition-all duration-300 flex-shrink-0 shadow-md">
                            <AvatarImage src={user.companyLogo || user.avatar} alt={user.companyName} />
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <h3 className="font-bold text-gray-900 text-xl truncate group-hover:text-blue-900 transition-colors duration-300">{user.companyName}</h3>
                          </div>
                          <div className="flex items-center gap-2 text-sm text-gray-500 group-hover:text-gray-700 transition-colors duration-300">
                            ...
                            <span className="font-medium text-gray-600 truncate">{user.name}</span>
