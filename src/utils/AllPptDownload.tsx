...
const AllPptDownloader = () => {
  ...
  const adminQuestionsReducer = useSelector(
    (state: RootState) => state.question.insights
  );

  const chart1Reducer = useAppSelector((state: RootState) => state.chart.data);
  const chart2Reducer = useAppSelector((state: RootState) => state.chart.data2);
  const chart3Reducer = useAppSelector((state: RootState) => state.chart.data3);
  const chart4Reducer = useAppSelector((state: RootState) => state.chart.data4);
  const chart5Reducer = useAppSelector((state: RootState) => state.chart.data5);
  const chart6Reducer = useAppSelector((state: RootState) => state.chart.data6);
  const chart7Reducer = useAppSelector((state: RootState) => state.chart.data7);
    ...
    return new Promise<void>((resolve, reject) => {
      if (adminQuestionsReducer[0]?.question) {
        ...
        generateCommonUIElements(
          slide,
          adminQuestionsReducer[0]?.question,
          pptx
        );
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[1]?.question) {
        ...
        formattedChartData.forEach((retailer, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[1]?.question,
            pptx
          );
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[2]?.question) {
        ...
        Object.keys(productChartData).forEach((product, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[2]?.question,
            pptx
          );
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[3]?.question) {
        ...
        Object.keys(productRetailerMap).forEach((ddd, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[3]?.question,
            pptx
          );
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[4]?.question) {
        ...
        Object.keys(transformedData).forEach((key, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[4]?.question,
            pptx
          );
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[5]?.question) {
        ...
        Object.keys(retailers).forEach((retailer, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[5]?.question,
            pptx
          );
          slide.addChart(chartDataArray, chartProps);
        });
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[6]?.question) {
        ...
        Object.keys(retailers).forEach((retailer, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[6]?.question,
            pptx
          );
          slide.addChart(chartDataArray, chartProps);
        });
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[7]?.question) {
        ...
        chart8Data.forEach((item, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[7]?.question,
            pptx
          );
          slide.addChart(chartDataArray, chartProps);
          // console.log('chartDataArray::: ', chartDataArray);
    ...
    return new Promise<void>((resolve) => {
      if (adminQuestionsReducer[8]?.question) {
        ...
        transformedChartData.forEach((retailerData, index) => {
          ...
          generateCommonUIElements(
            slide,
            adminQuestionsReducer[8]?.question,
            pptx
          );
  ...
  const RunDemo = async () => {
    try {
      ...
      await generateSecondSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateThirdSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateFourthSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateFifthSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateSixthSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateSeventhSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateEightSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
      await generateNingthSlide(
        pptx.addSlide({ masterName: 'PLACEHOLDER_SLIDE' }),
        pptx
      );
