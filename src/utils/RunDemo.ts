...
export const calculatePricePerOunce = (item: any) => {
  ...
  if (item.Price_per_ounce !== undefined && item.Price_per_ounce !== null) {
    ...
    return parseFloat(
      (item.Price_avg_last_4_weeks / parseFloat(item.Ounces)).toFixed(2)
    );
  }
  return 0;
};
...
export const extractOunces = (item: any) => {
  if (item.Ounces !== undefined && item.Ounces !== null) {
    return parseFloat(item.Ounces);
  }
  return 0;
};
...
export const generateCommonUIElements = (
  ...
  slide.addText(value?.SlideNames?.[0]?.slide_title || value.name || '', {
    x: 0.2,
    y: 1,
    w: 12,
    h: 0.35,
  ...
  try {
    ...
    if (id === 0 && (viewBy === 'Brand' || viewBy === 'Retailer')) {
      ...
      if (viewBy === 'Brand') {
        ...
        const allProducts = new Set(data.map((item) => item.Product));
      ...
      switch (id) {
        case 1: {
          ...
          formattedChartData.forEach((retailer, index) => {
            ...
            let labels = [];
            retailer.data.datasets.map((dataset) => {
              labels.push(
